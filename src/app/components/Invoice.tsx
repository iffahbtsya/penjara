...
export function Invoice() {
  ...
  // Use real cart items if available, else show sample
  const items = cartItems.length > 0 ? cartItems : [
    { id: 1, name: "Teak Dining Table", price: 1200, qty: 1, image: "", cat: "kayu", size: undefined, color: undefined },
    { id: 11, name: "Classic Cotton Polo", price: 65, qty: 2, image: "", cat: "baju", size: undefined, color: undefined },
  ];
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="bg-green-50 border border-green-200 rounded-sm p-6 mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-green-700 mt-1">Thank you, {userName}. Your order has been received and is being processed.</p>
          </div>
        </div>

        {/* Invoice card */}
        <div className="bg-white rounded-sm shadow-sm overflow-hidden" id="invoice-content">
          <div className="bg-[#1C1A18] px-8 py-6 flex items-start justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-white font-mono text-sm">{orderId || "TGS-000000"}</div>
            </div>
          </div>
          ...
          <div className="px-8 py-5 bg-[#F7F3EE] grid grid-cols-2 sm:grid-cols-4 gap-4 border-b border-[#D4C8BC]">
            ...
            <div><p className="text-xs text-[#7A7167] mb-1">Customer</p><p className="text-sm text-[#1C1A18]">{userName}</p></div>
            <div><p className="text-xs text-[#7A7167] mb-1">Payment Status</p><span className="inline-block bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full">Paid</span></div>
            <div><p className="text-xs text-[#7A7167] mb-1">Delivery Status</p><span className="inline-block bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded-full">Processing</span></div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#D4C8BC]">
                  ...
                  <th className="text-right py-2 text-xs uppercase tracking-widest text-[#7A7167] font-medium">Unit Price</th>
                ...
                {items.map((item, i) => (
                  <tr key={i} className="border-b border-[#EDE8E0]">
                    <td className="py-3 text-[#1C1A18]">
                      {item.name}
                      {item.size && <span className="text-xs text-[#7A7167] ml-2">({item.size})</span>}
                    </td>
          ...
          <div className="bg-[#F7F3EE] px-8 py-5 text-xs text-[#7A7167] border-t border-[#D4C8BC]">
            <p>Your order is expected to arrive within 5–10 business days. For inquiries, contact us at hello@threegoodsales.com or call +60 3-1234 5678.</p>
            <p className="mt-2">Thank you for supporting prison-industry rehabilitation programs through your purchase.</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          ...
          <Button variant="outline" className="border-[#D4C8BC] text-[#1C1A18] rounded-sm flex items-center gap-2">
